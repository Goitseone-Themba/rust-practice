...
impl Block {
    pub fn new(index: usize, data: String, previous_block: &Block) -> Block {
        let timestamp: u64 = SystemTime::now().duration_since(UNIX_EPOCH)
            .unwrap()
            .as_secs();
...
fn mine_block(index: usize, timestamp: u64, data: &String, 
    previous_hash: &String) -> (usize, String) {
    let mut nonce: i32 = 0;
    loop {
        if nonce == 1000 {
...
pub fn calculate_hash(index: usize, timestamp: u64, data: &String, 
    previous_hash: &String, nonce: usize) -> String {
